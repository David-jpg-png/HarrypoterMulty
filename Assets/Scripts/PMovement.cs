...
public class PMovement : MonoBehaviour {
    ...
    public float spellCooldown = 0.4f;
    ...
    void Update() {
        ...
        // Cast spell on Left Click if Spell ID 2 is selected
        if (Input.GetMouseButtonDown(0) && canMove && SelectSpell.weaponID == 2) {
    ...
    private void CastSpell()
    {
        ...
        {
            proj = Instantiate(spellProjectilePrefab, spawnPos, spawnRot);
        }
        else
        {
            // Dynamic fallback: create a sphere primitive for immediate testing
            ...
            // Remove the default SphereCollider and replace with a trigger collider so it doesn't push objects
            Destroy(proj.GetComponent<Collider>());
            SphereCollider col = proj.AddComponent<SphereCollider>();
            col.isTrigger = true;
            
            // Color it glowing magic blue
            Renderer rend = proj.GetComponent<Renderer>();
            if (rend != null)
            {
                rend.material.color = Color.cyan;
                // If using URP/HDRP, enable emission
                rend.material.EnableKeyword("_EMISSION");
                rend.material.SetColor("_EmissionColor", Color.cyan * 2f);
            }
            
            // Add a light
            GameObject lightObj = new GameObject("SpellLight");
            lightObj.transform.SetParent(proj.transform);
        ...
        projRb.linearVelocity = playerCamera.transform.forward * projectileSpeed;
        
        // Add Projectile script
        SpellProjectile sp = proj.GetComponent<SpellProjectile>();
        if (sp == null)
        {
            sp = proj.AddComponent<SpellProjectile>();
        }
        sp.damage = 25f; // deals 25 damage per hit
    }
